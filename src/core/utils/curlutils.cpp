...
size_t CURLWriteStringCallback(char* data, const size_t size, const size_t nmemb, std::string* userp)
{
    ...
    const CURLParams& params, const CURLProgress* progressData)
{
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "reSource Xtractor v" VERSION_STRING);
...
static CURL* EasyInit()
{
    ...
    if (!curl)
    {
        Log("CURL Error: Easy init failed\n");
    }
...
static FILE* OpenFile(const char* filePath, const char* options)
{
    ...
    if (!file)
    {
        Log("CURL Error: Open file failed\n");
    }
...
static bool FileStat(const char* fileName, struct _stat64& stat)
{
    ...
    {
        Log("CURL: File status query failed\n");
        return false;
    }
    ...
    const curl_slist* slist, const CURLParams& params)
{
    ...
    if (!success)
    {
        Log("CURL: Upload of file '%s' failed; %s\n",
    ...
    const char* options, curl_off_t dataSize, void* userData, const CURLParams& params)
{
    ...
    if (res != CURLE_OK)
    {
        Log("CURL: Download of file '%s' failed; %s\n",
...
CURL* CURLInitRequest(const char* remote, const char* request,
    std::string& outResponse, curl_slist*& slist, const CURLParams& params)
{
